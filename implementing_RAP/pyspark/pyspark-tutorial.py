...
# At the start of our script, we type the following to create the spark application entry point,
# the beginning of the spark session for this script.
spark = SparkSession\
    .builder\
    .getOrCreate()
...
CCG_2.show() #Notice how the .show() is broken into steps compared to line 90 where .show() is applied in one line
...
# In this example, we take the PRESCRIBING_SETTING field and multiply it. We put the results of the multiplication in a new column, 'PRE_MULT'
...
# In the example above, you can see that we combined multiple steps into a single query. This feature is one of the best parts of PySpark and is called 'method chaining'. 
# The downside of the example above is that it is a very long line and is visually complex.
...
# You can extend this chains as long as needed but take care that they don't become difficult to 
# follow. Consider splitting this out into discrete, logical steps:
"""
CCG_3 = (
    CCG
        .join(CCG_2,  CCG['CCG CODE'] == CCG_2['NEW_CCG_CODE'], how='leftouter')
...
# Note that we rename some of them using .alias(). We want to be sure all fields follow the [NAMING_CONVENTION]
...
# Hence, if Connor attended 12 sessions in three different clinics this year, we would end up with three rows for Connor. One row for each clinic he attended, showing the date of the most recent visit.
...
# The granularity of the main table is (person * organisation). So if Connor has attended three clinics this year he will
