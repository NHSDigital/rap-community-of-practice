...
!["Action 1 - copy-release-to-public flowchart"](./imgs/flowchart-action-1.png "Action 1 - copy-release-to-public flowchart")
...
`token` is used to set the [Personal Access Token] that is used to configure git in the running environment. It is used not only to checkout the code but also in future git commands. By default, it uses `${{ github.token }}`, which is a [automatically generated PAT] with access scoped to the Action's current repository. We need to access another repository in later steps, so we use `${{ secrets.EXTERNAL_REPO_TOKEN }}`. This is stored in the [repositories' secretes].
...
1. It uses the [GitHub CLI] command [`gh release view`] without a specified release tag to read the latest release from the current GitHub repository. The `--json name` flag argument will result in the being a JSON object that only includes the release name.
...
!["Git Process Overview"](./imgs/overview-of-publishing-workflows-push.png "Git Process Overview")
...
!["Release Process Flowchart"](./imgs/flowchart-push.png "Release Process Flowchart")

### Action 1 - [Push Release to Public RAP CoP Repo]

!["Action 1 - push-release-to-public flowchart"](./imgs/flowchart-action-1-push.png "Action 1 - push-release-to-public flowchart")
...
1. It uses the [GitHub CLI] command [`gh release view`] without a specified release tag to read the latest release from the current GitHub repository. The `--json name` flag argument will result in the being a JSON object that includes the release name.
2. The returned JSON object is then piped into the [jq] command-line JSON processor, which extracts the name key, with the `-r` flag returning it as a raw string with no quotation marks.
...
[Copy Release to Public RAP CoP Repo]: ./copy-release-to-public.yml
